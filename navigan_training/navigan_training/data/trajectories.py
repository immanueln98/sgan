"""TrajectoryDataset + seq_collate ported from naviGAN_paper/sgan/data/trajectories.py.

Returns 9-tuple including `goals` and `goals_rel` (exit-point proxy goals).
"""
import logging
import math
import os
...
class TrajectoryDataset(Dataset):
    """Loader for ETH/UCY trajectory files: `<frame_id> <ped_id> <x> <y>`."""
    ...
    def __init__(self, data_dir, obs_len=8, pred_len=12, skip=1,
        ...
        all_files = sorted(os.path.join(data_dir, p) for p in os.listdir(data_dir))
        num_peds_in_seq = []
        seq_list, seq_list_rel = [], []
        goal_list, goal_list_rel = [], []
        loss_mask_list, non_linear_ped = [], []
        ...
        for path in all_files:
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                ...
                for ped_id in peds_in_curr_seq:
                    ...
                    rel_curr_ped_seq[:, 1:] = curr_ped_seq[:, 1:] - curr_ped_seq_start
        ...
        self.goals = torch.from_numpy(goal_list).float()
        self.goals_rel = torch.from_numpy(goal_list_rel).float()
